...
//				int totalCombinations = (int) Math.pow(2, featuresReachableInCode.size());
				...
				{
					IFDSSolver<Unit, ?, SootMethod,?> solver = new SPLSingleConfigIFDSSolver(problem,new BitSet());
					if(debug)
						System.err.println("Starting standard IFDS solver for EMPTY configuration...");
					before = System.currentTimeMillis();
					solver.solve();
					long duration = System.currentTimeMillis()-before;
					log("EMPTY_CONFIG;"+duration);
				}
				{
					...
					IFDSSolver<Unit, ?, SootMethod,?> solver = new SPLSingleConfigIFDSSolver(problem,new BitSet());
					if(debug)
						System.err.println("Starting standard IFDS solver for FULL configuration...");
					before = System.currentTimeMillis();
					solver.solve();
					long duration = System.currentTimeMillis()-before;
					log("FULL_CONFIG;"+duration);
				}
